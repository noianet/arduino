/***********************************************************************
    Board: Adafruit Feather 32U4 (pro micro 3.3v), 8Mhz internal Atmega32u4
    Ref: https://forum.arduino.cc/index.php?topic=409415.0
    REMEMBER: disable ModemManager if "double-reset" flashing fails.
...
#define SLEEPLOOPS 337 //*8s pr to get roughly 49min sleep. Adjusted clockdrift NodeID 251: 326, Nodeid 252: 337, NodeID 253: 339
#define MY_NODE_ID 252 //<============================= Unique NodeID
...
void setup() {
    ...
    //Serial.end();
    power_adc_disable(); //Redundant really since re-enabled again in readVoltage() below. Here only for completion
    ...
    */
    ...
    // update sensor measurements after powersave stuff to verify
    power_adc_enable();
    CoreSensor.process();
    power_adc_disable();
    ...
    sendMessage(String(MY_NODE_ID)  + "@" + String(MY_NODE_ID) + "/255/3/0/12_6.4"); //Sketch version
    ...
    sendMessage(String(MY_NODE_ID)  + "@" + String(MY_NODE_ID) + "/255/1/0/0_" + CoreSensor.getTemperature());
...
void loop() {
    ...
    if (nbiot.isConnected()) {
        ...
        if (msgBuffer0 != "") {
            ...
            sleep(2, CHANGE, 1000); //interrupt pin, interrupt type, delay in ms. In practice heartbeat towards NBIOT. Wake if nrf get packet
        }
    }
        ...
        sleep(2, CHANGE, 5000); //interrupt pin, interrupt type, delay in ms
    }
    ...
    if (millis() - previousMillis > 660000) { //run every 11 min, holds in sleep for 49
        ...
        sendMessage(String(MY_NODE_ID)  + "@" + String(MY_NODE_ID) + "/255/3/0/14_1"); //Indicate gateway ready
        sendMessage(String(MY_NODE_ID)  + "@" + String(MY_NODE_ID) + "/255/1/0/0_" + CoreSensor.getTemperature()); //Send tempt
        hourCounter++;
